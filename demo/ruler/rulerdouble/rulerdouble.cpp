    ...
    for (int i = maxValue; i >= minValue; i = i - shortStep) {
        if (i % longStep == 0) {
            ...
            if (scaleStyle == ScaleStyle_Single) {
                ...
                painter->drawText(textLeftPot, text);
            }
        }
    ...
    for (int i = maxValue; i >= minValue; i = i - shortStep) {
        if (i % longStep == 0) {
            ...
            if (scaleStyle == ScaleStyle_Single) {
                ...
                painter->drawText(textRightPot, text);
            }
        }
...
void RulerDouble::setRangeRight(double minValueRight, double maxValueRight)
{
    ...
    //如果目标值不在范围值内,则重新设置目标值
    //值小于最小值则取最小值,大于最大值则取最大值
    if (valueRight < minValueRight) {
        setValueRight(minValueRight);
    } else if (valueRight > maxValueRight) {
        setValueRight(maxValueRight);
    }
...
{
    //值和当前值一致则无需处理
    if (valueRight == this->valueRight) {
        return;
    }

    //值小于最小值则取最小值,大于最大值则取最大值
    if (valueRight < minValueRight) {
        valueRight = minValueRight;
    } else if (valueRight > maxValueRight) {
        valueRight = maxValueRight;
    }
