...
void ProgressRing::drawValue(QPainter *painter)
{
    ...
    QString text;
    if (showPercent) {
        double percent = (currentValue - minValue) / (maxValue - minValue) * 100.0;
        text = QString("%1%").arg(percent, 0, 'f', precision);
    } else {
        text = QString("%1").arg(currentValue, 0, 'f', precision);
    }

    //如果定义了显示的文字则优先显示
    text = text.isEmpty() ? text : this->text;
