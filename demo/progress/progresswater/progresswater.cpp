...
void ProgressWater::drawValue(QPainter *painter)
{
    ...
    if (showPercent) {
        text = QString("%1%").arg((double)(value - minValue) / (maxValue - minValue) * 100, 0, 'f', 0);
    }
...
double ProgressWater::getBorderPadding() const
{
...
void ProgressWater::setBorderPadding(double borderPadding)
{
    if (this->borderPadding != borderPadding) {
