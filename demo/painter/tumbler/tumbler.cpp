...
void Tumbler::drawText(QPainter *painter, int index, int offset)
{
    ...
    int width = this->width();
    int height = this->height();
    QString text = listValue.at(index);
    ...
    if (horizontal) {
        ...
        painter->drawText(QRect(initX, 0, textWidth, height), Qt::AlignCenter, text);
        ...
        painter->drawText(QRect(0, initY, width, textHeight), Qt::AlignCenter, text);
