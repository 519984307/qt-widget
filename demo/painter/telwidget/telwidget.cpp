...
void TelPanel::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (pressedTime.time().msecsTo(now.time()) > 600) {
        return;
    }
...
void TelPanel::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (pressedY != currentY) {
        ...
        QWheelEvent wheelEvent(QPointF(0, 0), QPointF(0, 0), QPoint(0, 0), QPoint(0, -scal),
                               Qt::LeftButton, Qt::NoModifier, Qt::ScrollBegin, false);
#else
...
void TelWidget::initForm()
{
    ...
    this->setInfo(listName, listType, listTel, listPix);
}
                        ...
                        const QStringList &tels, const QList<QPixmap> &pixmaps)
{
    ...
    if (countName == countType && countType == countTel && countTel == countPix) {
        ...
        //对信息集合进行升序排序
#if (QT_VERSION >= QT_VERSION_CHECK(6,0,0))
        std::sort(telInfos.begin(), telInfos.end());
#else
...
void TelWidget::deleteInfo(const QString &name, const QString &tel, bool one)
{
    if (names.isEmpty()) {
        return;
    }

    //找到对应的联系人进行删除 两个条件都要符合
    QStringList listName = names.split("|");
    QStringList listType = types.split("|");
    QStringList listTel = tels.split("|");
    for (int i = 0; i < listName.count(); i++) {
        if (listName.at(i) == name && listTel.at(i) == tel) {
            listName.removeAt(i);
            listType.removeAt(i);
            listTel.removeAt(i);
            pixmaps.removeAt(i);
    ...
    setInfo(listName, listType, listTel, pixmaps);
}

void TelWidget::clearInfo()
{
    if (names.isEmpty()) {
        return;
    }
