...
void CustomPie::drawPie(QPainter *painter)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (showPercent && percent > 7) {
            text = QString("%1%2%3%").arg(text).arg(text.isEmpty() ? "" : "\n").arg(QString::number(percent, 'f', 0));
        }
        ...
        QFont font;
        font.setPixelSize(text.isEmpty() ? 20 : 17);
...
QString CustomPie::getInfos() const
{
    return this->infos;
}
...
void CustomPie::setDataPie(const QList<QString> &labels, const QList<double> &values)
{
    this->labels = labels;
    this->values = values;
    this->update();
...
void CustomPie::setInfos(const QString &infos)
{
    if (this->infos != infos) {
        //转为数据,格式:标签,值|标签,值
        labels.clear();
