...
CustomRing::CustomRing(QWidget *parent) : QWidget(parent)
{
    ...
    setOutPieInfos("直接访问,60|邮件营销,30|联盟广告,10");
    setInPieInfos("视频广告,40|搜索引擎,30|其他广告,30");
...
void CustomRing::drawLegendText(QPainter *painter, qreal scale)
{
    ...
    int maxwidth = 0;
    for (int i = 0; i < inPieInfo.size(); i++) {
        QString text = inPieInfo.at(i).text;
...
int CustomRing::getTitleFontSize() const
{
...
int CustomRing::getLegendFontSize() const
{
    return this->legendFontSize;
}

int CustomRing::getHoverOffset() const
{
    return this->hoverOffset;
}

int CustomRing::getHoverAlpha() const
{
...
void CustomRing::setTitleFontSize(int titleFontSize)
{
...
void CustomRing::setLegendFontSize(int legendFontSize)
{
...
void CustomRing::setHoverOffset(int hoverOffset)
{
...
void CustomRing::setHoverAlpha(int hoverAlpha)
{
