...
PathStrokeControls::PathStrokeControls(QWidget *parent, PathStrokeRenderer *renderer, bool smallScreen) : QWidget(parent)
{
    m_renderer = renderer;
...
void PathStrokeControls::layoutForDesktop()
{
    ...
    penWidthSlider->setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Fixed);
    penWidthSlider->setRange(0, 500);

    QRadioButton *curveMode = new QRadioButton;
    QRadioButton *lineMode = new QRadioButton;
    curveMode->setText(tr("Curves"));
    lineMode->setText(tr("Lines"));

    QWidget *pathModeWidget = new QWidget;
    QHBoxLayout *pathModeLayout = new QHBoxLayout(pathModeWidget);
    pathModeLayout->setContentsMargins(0, 0, 0, 0);
    pathModeLayout->addWidget(curveMode);
    pathModeLayout->addWidget(lineMode);
    ...
    mainGroupLayout->setContentsMargins(0, 0, 0, 0);
    mainGroupLayout->addWidget(m_capGroup);
    mainGroupLayout->addWidget(m_joinGroup);
    mainGroupLayout->addWidget(m_styleGroup);
    mainGroupLayout->addWidget(penWidthSlider);
    mainGroupLayout->addWidget(pathModeWidget);
    ...
    connect(penWidthSlider, SIGNAL(valueChanged(int)), m_renderer, SLOT(setPenWidth(int)));
...
void PathStrokeControls::layoutForSmallScreens()
{
    ...
    m_capGroup->layout()->setContentsMargins(0, 0, 0, 0);
    m_joinGroup->layout()->setContentsMargins(0, 0, 0, 0);
    m_styleGroup->layout()->setContentsMargins(0, 0, 0, 0);
