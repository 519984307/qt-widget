...
void NavDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    } else if (!node->image.isEmpty()) {
        int size = fontSize;
        int offset = (iconRect.height() - size) / 2;
        QImage img = QImage(node->image);
        img = img.scaled(QSize(size, size), Qt::KeepAspectRatio, Qt::SmoothTransformation);
        painter->drawImage(iconRect.x(), iconRect.y() + offset, img);
    } else if (parent) {
        //父节点左侧图标
        int icon = node->expand ? 0xf067 : 0xf068;
        painter->drawText(iconRect, Qt::AlignLeft | Qt::AlignVCenter, QChar(icon));
    }

    //绘制父节点右侧图标,存在子节点才需要绘制
    iconRect.setRight(optionRect.width() - 10);
    if (!(nav->getTipVisible() && !node->tip.isEmpty()) && nav->getRightIconVisible() && parent && node->children.count() > 0) {
        int icon = node->expand ? 0xf054 : 0xf078;
        painter->drawText(iconRect, Qt::AlignRight | Qt::AlignVCenter, QChar(icon));
    }
}
        ...
        if (parentText.isEmpty()) {
            ...
            for (int j = 0; j < count; j++) {
                ...
                if (childParentText == text) {
                    ...
                    //文件存在说明是图片
                    if (QFile(childIcon).exists()) {
                        childNode->image = childIcon;
                    } else {
